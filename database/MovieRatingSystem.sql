...
--
-- Temporary view structure for view `hot_movies_view`
--

DROP TABLE IF EXISTS `hot_movies_view`;
/*!50001 DROP VIEW IF EXISTS `hot_movies_view`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `hot_movies_view` AS SELECT 
 1 AS `movieID`,
 1 AS `title`,
 1 AS `releaseYear`,
 1 AS `cover_url`,
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_movie_rating_after_insert` AFTER INSERT ON `reviews` FOR EACH ROW BEGIN
    UPDATE movies m
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_movie_rating_after_update` AFTER UPDATE ON `reviews` FOR EACH ROW BEGIN
    UPDATE movies m
    SET m.rating = (SELECT AVG(r.Rating) FROM reviews r WHERE r.MovieID = NEW.MovieID)
    WHERE m.movieID = NEW.MovieID;
END */;;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_movie_rating_after_delete` AFTER DELETE ON `reviews` FOR EACH ROW BEGIN
    UPDATE movies m
...
--
-- Temporary view structure for view `v_all_review_details`
--

DROP TABLE IF EXISTS `v_all_review_details`;
/*!50001 DROP VIEW IF EXISTS `v_all_review_details`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_all_review_details` AS SELECT 
 1 AS `ReviewID`,
 1 AS `UserID`,
 1 AS `MovieID`,
 1 AS `Comment`,
...
--
-- Temporary view structure for view `v_review_details`
--

DROP TABLE IF EXISTS `v_review_details`;
/*!50001 DROP VIEW IF EXISTS `v_review_details`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_review_details` AS SELECT 
 1 AS `ReviewID`,
 1 AS `MovieID`,
 1 AS `UserID`,
 1 AS `Comment`,
...
/*!50001 CREATE VIEW `v_user_reviews` AS SELECT 
 ...
 1 AS `ReviewDate`,
 1 AS `movie_title`,
 1 AS `movie_cover_url`*/;
SET character_set_client = @saved_cs_client;

--
-- Final view structure for view `hot_movies_view`
--
...
/*!50001 VIEW `hot_movies_view` AS select `v_movie_details`.`movieID` AS `movieID`,`v_movie_details`.`title` AS `title`,`v_movie_details`.`releaseYear` AS `releaseYear`,`v_movie_details`.`cover_url` AS `cover_url`,`v_movie_details`.`rating` AS `rating`,`v_movie_details`.`director_name` AS `director_name`,`v_movie_details`.`actors_list` AS `actors_list` from `v_movie_details` order by `v_movie_details`.`releaseYear` desc,`v_movie_details`.`rating` desc limit 5 */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `v_all_review_details`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_all_review_details` AS select `r`.`ReviewID` AS `ReviewID`,`r`.`UserID` AS `UserID`,`r`.`MovieID` AS `MovieID`,`r`.`Comment` AS `Comment`,`r`.`Rating` AS `Rating`,`r`.`ReviewDate` AS `ReviewDate`,`u`.`username` AS `username`,`m`.`Title` AS `movie_title` from ((`reviews` `r` join `users` `u` on((`r`.`UserID` = `u`.`UserID`))) join `movies` `m` on((`r`.`MovieID` = `m`.`MovieID`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_movie_details` AS select `m`.`MovieID` AS `movieID`,`m`.`Title` AS `title`,`m`.`ReleaseYear` AS `releaseYear`,`m`.`Duration` AS `duration`,`m`.`Genre` AS `genre`,`m`.`Language` AS `language`,`m`.`Country` AS `country`,`m`.`Synopsis` AS `synopsis`,`m`.`cover_url` AS `cover_url`,`m`.`Rating` AS `rating`,(select group_concat(`d`.`Name` separator ' / ') from (`moviedirectors` `md` join `directors` `d` on((`md`.`DirectorID` = `d`.`DirectorID`))) where (`md`.`MovieID` = `m`.`MovieID`)) AS `director_name`,(select group_concat(`a`.`Name` separator ' / ') from (`movieactors` `ma` join `actors` `a` on((`ma`.`ActorID` = `a`.`ActorID`))) where (`ma`.`MovieID` = `m`.`MovieID`)) AS `actors_list` from `movies` `m` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `v_review_details`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_review_details` AS select `r`.`ReviewID` AS `ReviewID`,`r`.`MovieID` AS `MovieID`,`r`.`UserID` AS `UserID`,`r`.`Comment` AS `Comment`,`r`.`Rating` AS `Rating`,`r`.`ReviewDate` AS `ReviewDate`,`u`.`username` AS `username`,`u`.`avatar_url` AS `user_avatar_url` from (`reviews` `r` join `users` `u` on((`r`.`UserID` = `u`.`UserID`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_user_reviews` AS select `r`.`ReviewID` AS `ReviewID`,`r`.`UserID` AS `UserID`,`r`.`MovieID` AS `MovieID`,`r`.`Comment` AS `Comment`,`r`.`Rating` AS `Rating`,`r`.`ReviewDate` AS `ReviewDate`,`m`.`Title` AS `movie_title`,`m`.`cover_url` AS `movie_cover_url` from (`reviews` `r` join `movies` `m` on((`r`.`MovieID` = `m`.`MovieID`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-06-25 18:29:14
